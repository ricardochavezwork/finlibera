//TODO : Creazione costruttori di struttura.
//TODO : Creazione Default Values PDF. Ex. Header , Footer, Background ect.

/**
 * [TSDOC_PDF description]
 * @param {[Intestatario_DocumentoFiscale]} intestatario_DocumentoFiscale [description]
 * @param {[PDF]} pdf                           [description]
 */
function TSDOC_PDF(intestatario_DocumentoFiscale, pdf){
  this.intestatario_DocumentoFiscale = intestatario_DocumentoFiscale;
  this.pdf = pdf;
}

(function(){

  function table(widths, body, layout, lineHeight, style, bold){
    this.widths = widths;
    this.body = body;
    this.layout = layout;
    this.lineHeight = lineHeight;
    this.style = style;
    this.bold = bold;
  }

  table.prototype.getTable = function(){
    var self = this;
    var t = {
      table : {
        widths : [],
        body : []
      }
    };

    if (this.widths) {
      t.table.widths.push(this.widths);
    }

    if(this.body){
      t.table.body.push(this.body);
    }

    if (this.layout) {
      t.layout = this.layout;
    }

    if(this.lineHeight){
      t.lineHeight = this.lineHeight;
    }

    if(this.style){
      t.style = this.style;
    }

    if(this.bold){
      t.bold = this.bold;
    }

    return t;
  }

  function leftRowBody(text, style, margin, bold, color, aligment){
    this.text = text;
    this.style = style;
    this.margin = margin;
    this.bold = bold;
    this.color = color;
    this.aligment = aligment;
  }

  leftRowBody.prototype.setDefaultStyle = function(){
    this.color = '#5e5e5e';
    this.aligment = 'left';
  }

  leftRowBody.prototype.getRow = function(){
    var self = this;
    var leftRowBody = [ '', ''];

    var clone = {};

    if (this.text) {
      clone.text = this.text;
    }

    if(this.color){
      clone.color = this.color;
    }

    if(this.aligment){
      clone.aligment = this.aligment;
    }

    if(this.margin){
      clone.margin = this.margin;
    }

    if(this.bold){
      clone.bold = this.bold;
    }

    if (clone) {
      leftRowBody.push(clone);
    }

    return leftRowBody;
  }

  var emptyRow = [ '', '', ''];

  var Style = {
    0 : {
      title : "Finlibera",
      content : {
          textFooter : {
              color: '#444444',
              bold: true,
          },
          contenuto : {
              fontSize: 9
          }
      }
    },
    1 : {
      title : "Ecolibera",
      content : {
          textFooter : {
              color: '#444444',
              bold: true,
          },
          contenuto : {
              fontSize: 9
          }
      }
    }
  };

  var Backgrounds = {
    0 : {
      title : "Finlibera",
      content : "data:image/png;base64," + "" + "" + "" + "" + "" + "" + "" + "" + ""
    },
    1 : {
      title : "Ecolibera",
      content : "data:image/png;base64," + "" + "" + "" + "" + "" + "" + "" + "" + "" + "" + "AAAAAAwJYQBQAAAAAAALaEKAAAAAAAALAlRAEAAAAAAICtAFFxEr5GvvO5AAAAAElFTkSuQmCC"
    }
  };

  var Logos = {
    0 : {
      title : "Finlibera",
      content : "data:image/jpeg;base64,/9j/4QAYRXhpZgAASUkqAAgAAAAAAAAAAAAAAP/sABFEdWNreQABAAQAAABkAAD/4QMdaHR0cDovL25zLmFkb2JlLmNvbS"
      + "94YXAvMS4wLwA8P3hwYWNrZXQgYmVnaW49Iu+7vyIgaWQ9Ilc1TTBNcENlaGlIenJlU3pOVGN6a2M5ZCI/PiA8eDp4bXBtZXRhIHhtbG5zOng9ImFkb2JlOm5zOm1"
      + "ldGEvIiB4OnhtcHRrPSJBZG9iZSBYTVAgQ29yZSA1LjUtYzAyMSA3OS4xNTU3NzIsIDIwMTQvMDEvMTMtMTk6NDQ6MDAgICAgICAgICI+IDxyZGY6UkRGIHhtbG5zOnJ"
      + "kZj0iaHR0cDovL3d3dy53My5vcmcvMTk5OS8wMi8yMi1yZGYtc3ludGF4LW5zIyI+IDxyZGY6RGVzY3JpcHRpb24gcmRmOmFib3V0PSIiIHhtbG5zOnhtcE1NPSJodHRw"
      + "Oi8vbnMuYWRvYmUuY29tL3hhcC8xLjAvbW0vIiB4bWxuczpzdFJlZj0iaHR0cDovL25zLmFkb2JlLmNvbS94YXAvMS4wL3NUeXBlL1Jlc291cmNlUmVmIyIgeG1sbnM6e"
      + "G1wPSJodHRwOi8vbnMuYWRvYmUuY29tL3hhcC8xLjAvIiB4bXBNTTpEb2N1bWVudElEPSJ4bXAuZGlkOkZCMUQ1NENBMjI3NjExRTY4MEIxQzFCMjU4NjdDQzFGIiB4bX"
      + "BNTTpJbnN0YW5jZUlEPSJ4bXAuaWlkOkZCMUQ1NEM5MjI3NjExRTY4MEIxQzFCMjU4NjdDQzFGIiB4bXA6Q3JlYXRvclRvb2w9IkFkb2JlIFBob3Rvc2hvcCBDQyAyMDE0"
      + "IFdpbmRvd3MiPiA8eG1wTU06RGVyaXZlZEZyb20gc3RSZWY6aW5zdGFuY2VJRD0iMTdCRTg3MkNFMDlEQzc0OTdEMjUzNDc4M0I4RUQ4OTAiIHN0UmVmOmRvY3VtZW50SUQ"
      + "9IjE3QkU4NzJDRTA5REM3NDk3RDI1MzQ3ODNCOEVEODkwIi8+IDwvcmRmOkRlc2NyaXB0aW9uPiA8L3JkZjpSREY+IDwveDp4bXBtZXRhPiA8P3hwYWNrZXQgZW5kPSJyIj"
      + "8+/+4ADkFkb2JlAGTAAAAAAf/bAIQAAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQICAgICAgICAgICAwMDAwMDAwMDAwEBAQEBAQECAQECA"
      + "gIBAgIDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMD/8AAEQgATgDzAwERAAIRAQMRAf/EAMwAAQACAgMBAQEAAAAAAAAAAAAICgcJ"
      + "BQYLBAECAQEAAgIDAQEAAAAAAAAAAAAABQYHCAIDBAkBEAAABgIBAgMEBAgJDAMAAAABAgMEBQYABwgRCRITFCEVFgoxItYXQVEjWJgZWRoyk9Mk1FaX1xhxgZGhUjM0hFW"
      + "VlirV2JkRAAEDAwIDAwULBwoEBwAAAAEAAgMRBAUSBiETBzFBIlFhcRQVkdEyUtIjk1UWFwiBQpJT05TUseFicjMkVFZXGIJDpJXwwbJjoyWl/9oADAMBAAIRAxEAPwC/xh"
      + "EwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRYHunKfjFreVdwexORuh6FNsFPKfQ9029r6rSrJUCgcU3cfOWFi7bKeAwD0OQB"
      + "6D1ywWe0t1ZGBt1j8ZkJ7ZwqHx28z2keZzWEH8hUBdbq2vZXDrO8yVhDdtNCx9xCx4Pna54I/KFyevORnHvbsieI1RvbTWzpZJuo7Ui9ebPpF0kU2qQlKq5Oyrc5JOSN0xM"
      + "AGOJfCAiHUc82Q2/n8THzcpY3ltFWlZYZIxXyVe0Cq9llm8Nkn8vHXdrPJStI5WPNPLRriVmXIhSaYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRM"
      + "IsA8muTOpOI+n7JuzdE+MLU6+CbVoxZJpO7HbbG9IsaHp1PiFHDUZmzTR25/JR8xNFFFNVy5VQaIOF0rDtXa2a3nnIdv4CIy5CY+hrGj4UkjuOljRxc70AAuIBgN0bnwuzsH"
      + "PuHcEzYMZbtq5x7SfzWMH5z3Hg1o7T5qlUJu4P3keYnMaRmYCDtczoTRzoi7aP1TrmafRjuViHJhOmOwri0TjZy4vHTUyfmofzaJDwgZJmUwnOf6A7C6G7S2Vasu5WR5HPDi"
      + "64kaHBjhwIhjNWx6TXjxkrUF3cNBd69ddz70v5LJhkx+B7G27SWuew8QZnihk1NIOn+zpQhvedBNmj3Llw4cu1VnThY5lFXDhRRddVQwiJjqKqGMc5xH6REREcyDe2ta8Fw2"
      + "zmxAWsbQM8yxMuaYgH6UtASknByrRQFWslDvnUZINlCj1Ko3eMlUXCKhRDr1KYBDplXuYKVa4VYe0HsPp8q2CwOVt7yNsdw1kjfI4Bw9NDXirEHbB+Zb5NcUrRX9Xc0Jyzcle"
      + "Obp0yjFrVNr+9t26xZncHTUl4ezvVSu79EM0FuqkXLrKLeWgmRq6bB4iqYL3z0cwueiffbfZHZZkAmjRphlPkcwcGE/HYBxJLmuPEZow1/KyMCJ7pIPiOcXOH9R7iT/AMLiR2"
      + "BpYBQ+iTq3aOvd2a7pu2tUW2GveuNgwDCz062wDn1MVNwskkCrZ0gYxU10FiD1TXbrETcNVyHRWTTVIchdQr6xu8bdyWF/G6K8ieWvY7tBH/ioIqCKEEggq4Me2Rgew1aRwXf"
      + "c8q5JhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCKgv3wudEnyL5s2vTsLKq/dJxclJTWEFFE8SLaT2WzOm12xanyBgAyr9vY2x4FuJhMkRnEAqiBBdr+"
      + "P6J/hn2ba7a2q3clywHM5QB+o8SyD/lMHk1D5x3edQB4AL56fiv3Te5/Ot2javcMTjgHPaOAfcPbqqfKI2EBvdqLu8BSv4VcReJ3dg4eMtTykuz1JzU44R7qCj79EJovJO86s"
      + "WmHL6jylyrCp2BbhWq4hK/DRlkFgkYYrFj4nANl2zNWtdSN17w6L7/lzMMbr3p9mJObyncGRzlo5zYpBq5UhcObQjTIHP8ADqDniW6YbZ2f1t6dwYeeRtl1FwsQhMreL5IGk8"
      + "l8rDp5sWkiKoOqMtb4qFrDpL5wdqPmFwveP3e0NYPp3X6bk6LDbWv03Vr14+TMq4TandSjVom9rLh2VsY5G0s3YuRL7QIIdBHKO1Opey9/Qg4S6a3IEVdbS0jnbwBPgJpIBXi"
      + "6IvZ56qj57YW9On1yY8/au9RDqNuYqyW7uJA+cA8BNODJRG/vDacVqFnIgSicBJ09o+wQybvbXtVs2xny1zWl3kWF7DC+IDiBfx/g/wAuVyaItNFsLtzOBwbxVwz5S7uB2KH2"
      + "BsHty7HsDl/VLLDTm2+PCUk4UXNX7RAgm62XRIcvpFVU42x14TTyaJ3CbZovEvDppiq9UEdaOvO04nWsW7bRgE7HNinoPhNdwjeePa13grQkhzQTRoWY8VdtuG0Hfx/L/P8Az"
      + "96sG92/vLVbtnuNf62qmpJDe/IHZ0O4s8NSgnFqrW6vTwfPoOOstkmW8LPSUk6nrHHrtY+LYtvMcFZulFXDby0CusadOOlt/v1k9+6YWuGtnBr5CNRL6Bxa0EtAo0guc40FWg"
      + "B1TpiNzb1xG15obS8Ln31xUsjbxcQOFfd4cFph/eRu5f8AsvG3/at9fZPMk/cdtD6+Z+nB8tQ33hj6vvPo3e8n7yN3MP2Xjb/tW+vsnj7jtofXzP04Plp94Y+r7z6N3vLZZws"
      + "7yvJndfGbntyI5C8MyapW4fa2quwqbSUJC+1E22PekRs+UnYcbBd6k891e7ho7JMHDVi+8n1wmUSH8mBqNufpliMTncThMTkhce0pnRueOW8RUdE1p0seK11k0Lm1pwPap3G7"
      + "qjv8fdZCSCaFlq3UQ8FpIoTwqPMtiPa/59L9x/jW85CuNOOdHKNNl2rXfwW6u3x+dUtajK5IhNlnvg6j9CPviDy/I9EPlGQEfMP4uhadvzZ7tj5wYV1w26PIZJrDNA8RcKU1v"
      + "7NPbXv7FJYDN2+4MeMjbNeyIvLaOFHAinaPyrYvlLU0mETCKpZ2vu7HzX2htLupy3JO2xe4Ne8QdS7X2rqvXkfUKHrZdT7uLLenCFdUt1VpvvU/vaErqLH1T1KSFAR84UlT+I"
      + "D7Cb76bYPF4/b4xUbre5yk8Ub5C58n9o2PxaXPpwLi6gLa9lR3UvEb1x+XnyEMNK48OL6H4ta/+krIO3u9Rv8A3x2buQ/OnjpQkeKW39YcjadpOvsZObre9CnYqvNOSs1OOE7"
      + "XrOtwiQzEVslZkVqpGODIFbguRx41QKj5LDpVY4zqZZbNy0vr9pcWbpiWh0PGkwAGiRx4GOtdQrWhHDjw+3GOdtmbc2lzLSF+kh/AgjT2jurqFPNxW4HtO7323ya7eXGXem95"
      + "9radt7Bqljf3WwMoOGrTaUkIzYNvr7VwnB19lHQ0f0i4lApiN0EiCYom6dTDmN+oGGs9v7xvsNj2GOzgla1rSS4irGOPFxJPEntKsmGycOZxcGUg/sZ2ah6CtiOU5SaYRMImET"
      + "CJhEwiYRMImETCJhEwiYRePi42JJ2m6Wm0zb9w/mbNaJ6xS794sou8eys3Lu5OReO1lRFVZ06eOjqKHMImMcwiPtHPrBtuWK0xtvaQgNiihYxoHYA1oAA9xfOHqdhZptxX93KC"
      + "50t3M6p873U9wcB5uC3KduDjZzf3VJr7u4RuGCl40/PNG71eF2jSabda8rKMTHauHcLbJyG94VixtBctTgoVdg+Kk4brFMUDkGJ6gb96c4m0G2upEbzi72Oo1W8ksT6HjpfG1x"
      + "bIw0IpR7SWuB71SdkdNepmRvjunpjLGzL2UlDpuI4pW1HAOZIWtdHIKgg1Y4amnvCt0cfuVHcvgWLOl8w+3vabt0R92v8AZOj7hpt2tLNvQFQMrOa9mNita85dyDkpzOlmcqxZ"
      + "AVTom0KAeEdNN17K6P3EjsjsDdMUArqbb3kV0NJrWjJ2wF4DR8EOje6o4yd63L2dvnrTAxuM6jbSlmNNLrmymtXahSlXwOnDCXGpcWyMbQ0Efl+jf3ZS7c/MSJRtlk48yOirpO"
      + "otHryY1Sox1Xa45VNE6SjCXq0USf1ku88Y/l1gjXJ1jlA5XBwHxGruI6x9QdrSmyjyDchZsJAE9Z2O87ZHaJ6eQF7aDgWjsV3v+lOxs/G2+Fg7H3bwCeTSFzfM6NuqCvlIYa9o"
      + "ce1Up+9D2rNW9s666sqtH5BSu25XakVabSpSbDSGtfsNGqMVJNYuAlpSfjZx9Fz5LFIC9bJCmyYG8yNXN5YFEoZsz0139f8AUWyubu6sW2sds5jOY2QuZI9wJc0NLQW6RpJq53"
      + "Bw4rG24MDbbDvLe0jvXTvmDnBjmaXNYCACSHEGpqOAbxB4KHHZ+sFg133VuCdgrT5zGPX/ACKotLerNjmTM4r+xXpqDZ49XwiHjbyNesjlBQo9QEqmejqVZxXGx8pFM0FrbR7x"
      + "X40fjafyOaCsh7Oy3rF1FEDXVX+Qqz/3tNkQ2jO7TxY3tK11e3LanrGlthtKyg8KwXnEqFs+xWVeLbPzMZMGKzgzf6ivp1QSObx+EemV7oPt6Pc3RrM4MSCC4vZ5oWyEag3VEx"
      + "pcQCCQK9lRXsqte+vG7LzavWbCZlkbp7Gwt45ZI2kNLqyPo0E1A1UpU19BUj/3nKq/mY3P+1xt/dbkB/tLv/r23/dn/tlNf7ucf9Q3X7zH+zT95yqv5mNz/tcbf3W4/wBpd/8A"
      + "Xtv+7P8A2yf7ucf9Q3X7zH+zUja33V5HnbwX7idt1zr27cc7VoDSHvGGs0bsdy7soylxr2wXLGWrszDQFQla1IV9SlCZNwkcyhjrgJDEFMetUvujDen+/ts4/K3UGTsspkNLm8"
      + "nSzTE+EOa5r3PDw4SiopSg41qrbjutv2+2DufJ4m1nxt/isfra4yh7tUrJyxzSxrdOkwnjxJJ7qcdVTjlZyYL2Q2G4g5N8gQv5+ep6sfaIbgvwbDPSfuxerlqhroFg+IjVsJwpV"
      + "vRC59P5oAbw9cy+zYmyT+IF2DONsvs79n+dyeSzl8znBuvRTTq7tXbThVYgd1E3/wD7eW54ZC5+1pz/ACOdrdr0coyaK/F08adlePasw6W7eneW3pqPW25ar3ILfGVraFLrt6go"
      + "6w8ueVTadZRVljG8qybS7eNqkrHoyCKDkpVSouV0wOA+E5g9uQmf6hfh/wBu5u7wN5tPXdWdw+F7mW1oWF0bi0luqRpoSOFQDTuU5t7Yn4i9x4K0z9nu+JlreW8czGvfcag2Roc"
      + "A6kRFRWhoSPOsm/qnu9r+0ulv0wOW/wBhMiPvb/Dt/lGT91s/2ymfup/Ep/nG3+kuf2K5ftyb85s8bu5rN9uXlHu+Z5BJTdclZR9J2K82XY5KxMoatbbYgbFUbpeoxndlIt9AJe"
      + "7ncW4BFqVy584iZTpGMtw6n7V6dbl6TRdUdlWJxj2SAcsMazWw3BtnB7I3FgcJPEHipIGkmh4fnS/d/UzbnVubpZvy9ZlInxkiYFx0SC2F03S57WuLTHwLTShNVDXtci7qVy709"
      + "ipcvLVy2QPG/kA9jrBBSDqInIKyMJrZMnES8NKx6zd/GSsY+QIu3cIqEWRWIU5DAYoDlw6t46zuMfsK0kY2SGXIWkcjXAOa4PbAHNc01BDgSCCKEGhVU6R53KRZLf14+RzHRWN1"
      + "NC4Egt5Zno4EcQQQCCOxfsds3Ym8vl/uUts3JtLZG3pFDmPU2DSy7Pu1kvs8yrsWXjy5ShWUtaZOVetIdrKuXa6bVNQqBF3KygFA6pxN2T7d2/gfxKYiwwVrBaYt2HfIWRMbG0u/"
      + "vYc4hoALjpAqeNAB2ALott2bqzv4Z8vkc7cy3OfblhBrkcXGhNo5rRXsFH1oOFST2krtXEft49zzdHGPSGx+M3N6Q03pSwUknwbrpvyL5C6+TgQjpqYjJtQKrQapJ1ZgMvYWjp6J"
      + "m65zL+o8xTooYwBH7v390a25uzJYndm3n5DNx3RLphBbPDmva18Y1SSNedLHNHEcKUHAAqU2lsrrfujaWLzG09xw47DPsmNELpJwQ+Nzo5DRkbmjU9p4AnyniSFIj9U93tf2l0t+"
      + "mBy3+wmV772/w7f5Rk/dbP8AbKf+6n8Sn+cbf6S5/YriODG9Oc/EXukwnAbk7yFnuQMVfos5LAWyXm37Vjot3Iazd7Cqdmolt2Ezi7pDrtF2hI+QZnInHrJrODeQoqm2cl9XUHa/"
      + "TbePSB/UjZmN9l3Vu+rWhrIy9jZxBI2RkZcw1rqY4HUKNGoAuafD0+3d1R2h1ib013zkWZWyuGUMg1O0SOgM8Zjc8NeOA0uBFDUmhIaVhTkXyM5r8y+f/KPTNC5uRHDTX/G2xXis"
      + "xQ2DfVm47a2ThaTeo7XTY0laKazNJWm8XWVW95FJJCqVBM6yLc6aSREzWDa+zuney+muIz2VwFzuDM5SOGR7YrdtzM0zQunGmNxoyJjBpJbSpoXAkkqvbo3z1L3l1My+38PuCy25"
      + "gcXJNGx883IifyZmwO1SU8Uj3nUATwFQOAAXC/4d+dP7eXjz/wDp1uP/AOPzu9sdNv8ATXOf9pj99dXqPVH/AFP27/3H+ZYd3g+558SKa23g07xFH3avWLLApEo+qOcd53dY1VHro"
      + "xEZJ/rO6shrVnrTNwQhXyLpF0j5agCdE5PF0ndvY7pfvK/dgbjYuRxcUsLyZ7jHtt2NoOwTNOpjz+aQQajga0UBuPN9Wdm49udtd+YjLzRzMAtre7E8j9R7TER4mD87geHaKVI2U9"
      + "xXukb6jO3dwfvWsZlfVmxuZFeklrxeagJY+VglKZEwMdaYuiquPXPKyNus9hBdq9RXF/HMmwppLeacFi4q6X9HdtXPU7PYzPA3WHwcrRFE7slMznui5lKB2mNhDmkaXuNSABRZW6p"
      + "9Z90WnS/A5XbYbbZzOROdJJ+pEIY2bQOJFZJBpdWrGjvK6Y07U3e2dtWzo3comm5nLdFwLdXmDyyFVAVkyqCioKFIVRFRITeEfAYxeoewRD256n9Wfw7MeWfZJ5oSKi1s+NO/jLX3"
      + "V0s6WfiUewP+2FuKgGnMueFe7hFRbBOB/B/uMaEX5AKcl+YT3c6OwdQvahq4jne28ti/A99WO+M2taYbCrUUWuGblXTD1cd5rv6v8D6pcxn1F3x0s3D7O+x2EdjvV7sPuKwW7ObFw"
      + "8HzcjtXYfC6jePasn9OdmdWNv8AtD7b5yLJme20W+mSY8qXj4jrjbQcR4m1dw7FXWuli5Ha4s0rSbZ3364SxV5x6CXJB8peetwjE3iRS+cm3sVR0tO1+R8Bh6GM3dKlKbqAiAgIBs"
      + "7YY3ZORtGXlv06uuTI2o5kOMhdTymOW6Y9tfI5oWr9/meoNheSWk3UexMsbiDyxkZmA+QSxWj43U7CWuPHzrq/3sbs/bws/wC3/uNf3AZ6/YGz/wDTqX9HEfxq8f2k35/qNb/Q5X+"
      + "BWY+OHOHlhxo5ScbHhudcxzG1JubYEJr+yM3F83ZsakSMe9tUHXrXCt2m/wCpUmzVq619nYmsgzkYpuVAVDpJqrOEfVsxidz9N9h7p2dlORt84HPWVu6aI0tmvJEb3scTayzRvjcY"
      + "3McxztXAkBp0uUntnqf1F2vvTFNu9wRbg25e3DYZqNnDWBz2McALmKGRsjRIx4c1ukggVd4mq9Pnz3X0MTCJhF49fKjVE9xd5b8iuPVk80JLU24LxU27lVo4YBLwTOddqViwNWrkC"
      + "qkYWKtrtHzYfaU6DghiiJRAR+kmzc9HlcFZZSI+Ce3jd21odI1Nr5WuqD5wtauo20mzXE0rW11Pc73TX/zUrOB0/wAyE9rJTHCNhuiS2zX4k0m9LpOJn52cQrQPmSDs9hi4Nq9SeV"
      + "ZZ8q3ScJPkVGKhzkIoUfEADadxT7MucPyN8mx9jPcG/wB6cxjNZBI0OeRSSgJBYQ4UJB4LWu2xW88ZmzNsoXwzTGk/3Zr3P0AiuprA7UypAcHgtNQCOKuQcUtyfMJ35uxJddNaaiY"
      + "p07K1cW3kXWGGv3kaQClFR07quvbJX7Yu1KAD7UYRQxjD7B6fRrHu3Bfhdx7HOtr7IOuw2oisJDOHebmTxyRA+mYUHcs27Tzv4rMhO1k1jjhYl1DNkIhAWjy8uCSOUj0QGp71vN0jO"
      + "X9JCRpG69papv8Au2HYxVht8FqOsSdTg6RD2AiycE0dQ85cLtYvLk3TB56KQkF2AyybVQ6DNIEFwDXDPwYwyNv8BaXltgHucyJ1zI2R8rmfCIcyKJlWgt1MYHcsuAc86mrZrAT5MR"
      + "Gxz93Z3OfY1r5W20bo2RB48I0vllfRxDtL3lvMDSWsGlyoMfMgBaZbud7PTnjySsREa60/H0hN6ZwLVCsq0SMk3hYcqvRIrA9ykZYx/K+oLsVuv1/F03d/D9bWp6YwPgDec65nMl"
      + "KV18wgavPywyle6nctSutWTurXqS+OUuELbaHl17NJbV2n/j1Vp3qMXYp47Se8+6hxkSbtZAYPUdhk98WuRZMzOU4iO1dGLzFfWfnAQI0ZSuwFIaNFQw+w74oAAmEAzh1ryEeF2J"
      + "elxbzrkCBgJoSZDR1PKRGHu/Ir50fupcrmGltTFBE57j3cRpaD5Kk8PQVvu7qFmbofMNdsyruyouG0q/0W2XZOSJrN3KMntWbaKJLIKgZNVNQodBKYBAQ9mYu6bZOS06IZyCNxa4"
      + "sveINCC63aBQhT29dle2epVjnnxh8UEljWoqCGXLnGteHAFW5/gSkf1Nqv/j0R/Q81i9p5L/ET/SO99Zu9lYv/AA1v9Gz3k+BKR/U2q/8Aj0R/Q8e08l/iJ/pHe+nsrF/4a3+jZ7y"
      + "hB3NWUJT+3LzfkoWIioby+Nm0nLk0ZHs4/wA70lUlDIiuLVFLzfJ80/h8XXw+Ien0jlw6d39yd/4ae4ke/l5CIjU4up4xWlSaVoPcVb3nhLa82dk8ZaRRsNzaPYQxobWoIFaAVpU0"
      + "9KpPO7//AOszGzXqPYPcjNEeZ4g/Brt+88PXr+MM2xblnffs65r4vs2Gf9SHLBh6cPPT5u3NBoM+Z+zu9ULK+7wV4jtmv/evb04VSXi8XruM2nHXi/2vOpEOfr/n65qJ1Gl5+/cxN"
      + "8bIzn3ZHLPuyLA4rZ+MxrhQwWUTKeTSwBTiylq0qoG1tv8A7b0jV/O+jWhE/K8QfR/ggYyv0dfxj1zaV2QP+2ZtjXw1p/8AqF6wQNnPHWg7t0+E3Hb5vZYhr7vBQq7U98GRtfzARv"
      + "UeP3Jxr5JSH8IB8sWbrbJQEPb7OnlZcuouVMzNlEnhFlrN3uCH3lVtq7AkxUO4joIOQxF3H6S90nD8upYy0NfPN+V85dz3qOoM+a8dGeZ4g+qCx+N5xL1/53/Xnpy2Vcev2Kuj8Ju"
      + "Dkb7rr331ztOnr4emmR21oPz2bhmpTtAjs+P/AMZ9xTZ7c3zIvAHitwm4+8ftrw3ISQ2FrKpSELZ3dPoFQlq2s9d2mfmkfdUjJ7Ig3zpJNjKJFOZRqj0VKYAASgBhofUPpJujd28r"
      + "7cVjPYttbl7C0SPlDxpiYw6g2FwHFp7HHhRZD2NdO2lta129LY3r323NGpghLSHzSSAjVM09jx2tHFTY/exu1/8A1a5U/wBl9B/veyl/cDvL/EY36Sb+HVs+1rfq/Ie5b/xC1lcWe"
      + "XdU7lvzF+teQPGuubBU1RFUs0tOrXCvIRkrW4SmaTkarKS1mbwcpY4uFYv7Y6bsmSh3hiLru26YiVVYEwyrmrefZHQa42rl5YTeGrBocS1zpbsShrNTWOcQyrneEUDXHiBVYmbtOX"
      + "MdWYd6CCSIc9r9L9OoMjtDCXO0Oe0Au4DxHtbWhNBB7kVsPi5prundzPW/cbjOSdU1bsrZ16mqunodhUE748WkdsRmxteTZhvY+6fgux0l0LsqyJTrqec38PQonELrg87uO66eYKX"
      + "ZL8e+/t7S3Y71rmmKkdsYJW/MkP5jZABQkDg6vcqjfdKcffbqyV7m4b02txd3b/7sYRKHS3QmjceeCwsdHUngXVLad5HX/vI+W0/693W/4rj1/I5z9v8AXH4u1v8Ar/lrv+57Y/6r"
      + "cn6WO+Qn3kfLaf8AXu63/Fcev5HHt/rj8Xa3/X/LT7ntj/qtyfpY75CmB3nr9pKL4B9l2R44P9iLaOlh3M51q42z7iJsZWvsZDV5UwtwVoicF70SduFSj6YPL8sC+0R6iNc6VZjMx"
      + "743VcZ71cZiSazMvI18mrWzj5vmEv00I+Ea1Xt3x05ssltfCYnEsuDYWNnfBgm0GUcx8Bo/lgMqXNd8EUp2K+vHD1j2I/jZtR/0oEHNKJOMjvSf5Vs9GKMaPIAo78zI+7y3ETlHF6"
      + "0aS0hsKR497jZUqPgCOFJ+RszrXtgRho+BIzAXZpx8+ORJmCXRT1JyeEQN0HLDs24s7Td2LusgWiyiyFu55dTSGtlYSXV4aQBV1eFK1UHuuzuchtfI2Fnq9ansZ426eDtT4nNGkji"
      + "HceBHEGi88rt8csuzBrnSc7Xu4px83XtHkB95VkeIWGmpzBotrR1YyvoREO7K23Hr7y5hpNtpJRcDMlDeFYnVYw9SJ7vb1uuq99k45tkZKytMULcB7JWMc50ut5c8F1tN4SwsAo4c"
      + "QfD3nXDbfSzZtraSM3Firy9ndNWJ8Mro2CLlxgM0tuYOIeJCfCeDh4u5s6v8d/yt/wCZnyX/AIiw/wD2dyn6fxEfXmM+jj/gVYvu26W/5eyf7zJ/HqFe8eXXb82tzI7f1b7duvtj"
      + "aq1VU910ta2VjYpHpHa93tm06SkvJxKj/YGw1vQOoaHapmTK6QTKsmYwI+I5znuGGut72O2Mu7fN3b3eSltpWsfC0NAibBJRpDYYRXW95rpJoe2gAFXy3S7COy1rc7ax9xZY+CSN"
      + "7myvMjnS86MlwJmmcBojYKagOHAVJJ9HN1sBqy2PH67dQ8q1UlYFOYjbG7TSaV+WenUnDLVyFeLnISXsMZH19Z49aIiLlu0WQXBM6JllEPn6ttF0CscgIix1l/ZV6zLVxNKk662B"
      + "FtbDL1RmEpXdqLy7GnCZ+lOrsGEm7koNdFVqc5lepkSogusqCIEXFf4mK/7v95/Bd49J90n3p+L3cz8fn/E3wl8B+D13i+L/AH19Twf8N4Pr+b0wiq1/M8dpi27VRQ7jXHGqPbFd"
      + "KPWWcFyapkC3F1Mz1ErTYUq9tqOjkUzPJN7SIhMI+aKkKihIZBq4KkCTN2qGxXRLfsNg/wCyGVkDIJHl1s5xoA9x8URPYNZ8TK9ry5tSXNCrW4bCKeAzSD5qlHH4vkcf6Pc49wo4"
      + "0AcVTR0HvzZemLjDbG1FsC2a1vcJ55Yu10ycf16eYFdoHaPUUpCOXQcA3dtlTJrJiIpqEMJTAIDm2RisMnaux2WhiuLGSmqOVrXsdQ1FWuBFQeINKg8RxWtO7dvXuMuvaGMklt72"
      + "OpZJE5zHtqCDRzSCAQSDQ8Rw7FYY4hdxTffIyd+HeW3ds21xp1/FNTeoexcLf5y2WhuYhSHj42R1vBoJt3K/XwmcSDoxkyiJipqj1INbzeydtYOw9c2ltDH5bLudwY4wRsZ5HO57"
      + "uI/oxt404lvArGFtubdOSypx+6t2ZPEYVjeMrDcSPf5WNEAqDTtfK6grwDzULben3ZeIvGOgJ8ZO2lGg/td0nxbWLlTyHXWhqoWyShGMe+27cXVgSbW/Yc00YJqKAk7YxTNt6dMr"
      + "do7S6MVcZv6Mb43hev3l1SeG2kEfzePsW65S0VLbaFsdY4GF1AXB8jiCS5wd4xeoutWw9l2keyelzC+7nk+dyF87lwtJ0h91O+Wks7w2pDS1gqGta0t8KiJ3JN/cGeb0M/h3dwtU"
      + "zPcUdBV+ia55LjElV3nyz3nOTUS2r1Mb6sk3EI6d6lXSjrHLzUu7RaLxcs/TM09ORVNjPe7Ym1+ovT24ZKbeKGPLXb5prJxPq1haMB1TSTDUGSgmOONgJ1sYQ4O+FFJ7o3R016j2"
      + "r4xcS3PsuFkEN7HT1i9u30+ZgiNDK0gOfIS3S1z2lpbTxblOwx2xZLg5pGd3DuKCNF8keQLKJXmoSQatySertasTC/r1BWEyZ3rCwTT1b3pPoCqUCLkZNFUirR5znwl1t6jR72zz"
      + "bHFO1YCyJaxwJpLIeD5fIW/mxmnwaurR6zX0o2Q/Zu3gL0f/AG90Q+WtKsH5kdR8UHxcaaiaUW4y08bePV42VX9zXTRmo7Zt2pjCDV9n2TXdTm9gVwazIOZeuDB2+RiXM9FjAyrx"
      + "Zyz8hcnpl1TqJ+ExhEcSQZnL2tk/HW11cR4+TVqjbI4Ru1AB2pgOk6gADUcQKFZOMUTniRzWl476CvDzrNWRq7Ewi65b6fU9g1eepF7rMDc6baot3CWaqWmIYT1csMM/SMg+iZqF"
      + "lEHUdJxzxEwkVRWTOmoUehgEM7re4ntJ2XNq98dzG4Oa5pLXNI7CCKEEdxC/HNa5pa4AtPcVhEeHHEk2tyadHjHoIdTJ2sb2TWY6ioY0Il2FgeKG3lqXuH3EWzDGKGb+uBD1PkCK"
      + "fj8I9Mk/tFn/AFz2j67d+v8AL0czmv16K10atWrTXjprSvGi6+RBo0aG6K1pQUr5aeVZwqVRqtBrEBSaNW4KnU6qRLGBrFVrEUxgq7XYOMbkaRsPCQ0Yg2jouMYNUypooIJkSTIU"
      + "ClKAB0yMuLie6nfc3T3SXEji5znEuc5x4kuJqSSe0niV2Na1oDWgBo7guw50r9WFyccOPpNvqcgiaP1KTe6xgOtuYuvaoXaSpwqqdGA6l9CK+JznClJFiOouuvu0oNv9yHgyS9sZ"
      + "Y4/2Sbq49lj/AJPMdyvha/gV0/D8fZ8LxdvFdfKi183S3meWgr2U7e3s4ehcHV+JXFmkDsIabxx0ZVB21CS9a2mNc1VSIb7x67YDvVJ2BvPu6Eb/ABVDzJ5JwZ02feei4FdQTlN4"
      + "zdeyfPZy55PrF5dSeruDotUrzy3NppcyrvCRQUIoRQUX4IYW10saNXbwHH0r52PD/ihGaznNLx3GnQzDUFnsza6WTVzPU1Fba+n7gzShUGlpmaejBkgJKwtUK3HETeLNzuCFYtwA"
      + "4Ain4eTtw55963JPvbo5BjNDZTK/mNYa1aH6tQadTuANPEfKU5EIYYwxugmtKClfLT8i6B+ro4A/mS8Uf0f9WfZbPX9r91/WV/8ATy/KXH1W2/Vs/RHvJ+ro4A/mS8Uf0f8AVn2W"
      + "x9r91/WV/wDTy/KT1W2/Vs/RHvLNep+PWhNCoy6Gj9Jal06lYTtFLAXV+uqjQvfyjAFwYqTZqvERZ5ZRkVyoCJnAqCkChgL0ARyMv8tlcqWnJ3NxcFldPNke/TXtpqJpWgrTtXYy"
      + "KKOvLa1tfIAP5Fxe1uMHGve8lGzW7ePulNvTUOxNGRE1szV1JvMxFRh1zujx0ZK2WEkn7Bgo5UMoZFJQiZjmEwl6j1zssM3mcWwx427ubeNxqRHK9gJ7KkNcAT5yvx8MUhrI1rj5"
      + "wCsUfq6OAP5kvFH9H/Vn2Wz3/a/df1lf/Ty/KXD1W2/Vs/RHvJ+ro4A/mS8Uf0f9WfZbH2v3X9ZX/wBPL8pPVbb9Wz9Ee8u82jhtxHvFQo+vrnxh0Ba6JrNlMxuuaZY9Q0Kaq1Dj"
      + "rEqzWn2FPgZGBcRdcZza0c3O7SaJIkcGQTFQDCQvTywbiz9tcS3dte3cd1MQZHtleHPLa6S9wdVxbU0JJpU0XJ0EDmhjmNLR2CgoPQpIEIVMpSEKUhCFAhCFAClKUoABSlKHQAKU"
      + "A6AGQ3au1f1hFFa2cFuFF8scxcLrxE4zWu2WF+4lLBZ7BozWcrYJ6UdnFR1Jzcw8rK0hLSLlQeqi7hRRU4/SYcnbfc+5LWFtvbZC9jt2CjWtmkDWgdgADqAeYcF0m3t3Eucxhce+"
      + "gXXf1dHAH8yXij+j/qz7LZ3fa/df1lf/AE8vyl+eq236tn6I95fZG9vnghDyUdMRPDLi3GS0RIMpaKk2GhtYNH8bKRrpJ7HSLB2hWSLtHzB4gRVFUhinTUIUxRAQAc4P3ZuiRhjk"
      + "yN86NwIIM8hBBFCCNXEEcCE9WtgaiNlfQFJaWo9LnpNCanKlWpiYbIt0G8rKQcY/kUEGjg7tqki9dNlXCabZ0qdRMAMAEOcwh0Ew9a+u9dLruhdOVitxdVjtb0xSLio6IjkzPqzC"
      + "O3rwsI2YNmD2SeKMPNfSZPdaCouD/lBXTKp1AwAIEX2fcdpXz/VfdDrD1Pk+m9R8A1XzvTe7fc/pvN91eP0/un+beX18Hp/yfTw+zCLKAgAgICACAh0EB9oCA/SAh+EBwirH9w/5"
      + "ZfjLygsc7t7i3YGvFXbsyd3IzFYYQYSOj7bMrrHcC8Vq8edq/wBfu3SqpvOViPOZeEpfBHlN4znzvsvrjlsHGyw3HG6+sW0AkDqXDG9nEu8MwHdrLX+WQigFEzuzfXY3HFPZHWvz"
      + "UgJiP9RzfFDXvoJIxTwxAkk6BbF8tx3T6FPPYyuUXV20IlmuKbOy1HbtQh2UogAAJXCMff3lOmGwG69BIsgQwGD8IdBHYXEdfOnLoWy3N7LbvI+BJbzlw9PKZKz3Hla47p6QbxvZ"
      + "nCzx3Md8Zk9vyz6DLLFJT+tG30LPulvl7e5TYZhFheK1qzUMYVMVV524bQgrA3DwCUBbtmOtPjmQXdnAREgKERRHp9ZUvsyXvPxN9OsTBzLGS6vp+5kcL2flLpxGAPRU+ZY2H4Xt/"
      + "Z245d4y2sLc9r5JWSH0BsJkJPpIHnVl7gf2XuO3DqZidn296ffW94sUHMRdrPCtouqUZ+ic6hX2v6R6qVTj5tMTEAJV+7fv0zJAdoLLzFUz6v8AU7r1uzqK12OaBYbePbBE4l0g/"
      + "wDek4F4/oANZxNQ7gVs90q6BbP6XtF7DqvtwkcbiUDweaFnERjz1LjQcR2LchmDFnRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImET"
      + "CJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCJhEwiYRMImETCL/9k=",
    },
    1 : {
      title : "Ecolibera",
      content : "data:image/jpeg;base64," + "" + "" + "" + "" + "" + ""
    }
  };

  var Footer_Options = {
    0 : {
      title : "Finlibera",
      color : "#5e5e5e",
    },
    1 : {
      title : "Ecolibera",
      color : "#5e5e5e",
    }
  };

  var Footer = {
    0 : {
      content : {
            stack: [
                    {  text:  [
                                  { text: 'Finlibera s.p.a. ', style: 'textFooter' },
                                  { text: 'Via Pier Lombardo, 30 - 20135 Milano MI', color: Footer_Options[0].color }
                                ],
                        alignment: 'center'
                      },
                    { text:  [
                                  //{ text: 'Tel./Fax: ', style: 'textFooter' },
                                  //{ text: '+39 0236743720 | ', color: Footer_Options[0].color },
                                  { text: 'Email: ', style: 'textFooter' },
                                  { text: 'segreteria@finlibera.it | ', color: Footer_Options[0].color },
                                  { text: 'PEC: ', style: 'textFooter' },
                                  { text: 'finlibera@pec.it | ', color: Footer_Options[0].color },
                                  { text: 'Web: ', style: 'textFooter' },
                                  { text: 'www.finlibera.it', color: Footer_Options[0].color }
                                ],
                                alignment: 'center'
                      },
                    { text:  [
                                  { text: 'P.IVA: ', style: 'textFooter' },
                                  { text: '07301400961 | ', color: Footer_Options[0].color },
                                  { text: 'R.E.A. ', style: 'textFooter' },
                                  { text: 'MI-1949464 | ', color: Footer_Options[0].color },
                                  { text: 'Capitale sociale: ', style: 'textFooter' },
                                  { text: '250.000 € | ', color: Footer_Options[0].color },
                                  { text: 'Sede legale: ', style: 'textFooter' },
                                  { text: 'Via Pier Lombardo, 30 - 20135 Milano', color: Footer_Options[0].color }
                                ],
                      alignment: 'center'
                      },
                    ],
            fontSize: 7,
            //height: 60,
      }
    },
    1 : {
      content : {
        stack: [
                  {   text:  [
                                { text: 'Ecolibera Srl, ', style: 'textFooter' },
                                { text: 'Via Pier Lombardo, 30 - 20135 Milano MI', color: Footer_Options[1].color }
                              ],
                      alignment: 'center'
                   },
                  {   text:   [
                                //{ text: 'Tel./Fax: ', style: 'textFooter' },
                                //{ text: '+39 0236743720 | ', color: PDF.Footer_Options[1].color },
                                { text: 'Email: ', style: 'textFooter' },
                                { text: 'ecolibera@legalmail.it | ', color: Footer_Options[1].color },
                                { text: 'Web: ', style: 'textFooter' },
                                { text: 'www.ecolibera.it', color: Footer_Options[1].color }
                              ],
                      alignment: 'center'
                   },
                  {   text:   [
                                { text: 'P.IVA: ', style: 'textFooter' },
                                { text: '03950960165 | ', color: Footer_Options[1].color },
                                { text: 'R.E.A. ', style: 'textFooter' },
                                { text: 'BG-422559 | ', color: Footer_Options[1].color },
                                { text: 'Sede legale: ', style: 'textFooter' },
                                { text: 'Via Angelo Mai,4 - 24121 Bergamo', color: Footer_Options[1].color }
                              ],
                      alignment: 'center'
                   },
                 ],
        fontSize: 7
      }
    }
  };

  PDF.prototype.setBackground = function(img){
    try {
      if (!img)
        throw new TypeError('PDF : setBackground - img must be defined');

      this.background = [
                {
                    image: img,
                    width: 592
                }
            ];

    } catch (e) {
      console.log(e.message);
    }
  };

  PDF.prototype.setHeader = function(img){
    try {
      if (!img)
        throw new TypeError('PDF : setHeader - img must be defined');
      this.header = {
                margin: [20, 20],
                columns: [
                    {
                        // usually you would use a dataUri instead of the name for client-side printing
                        // sampleImage.jpg however works inside playground so you can play with it
                        image: img,
                        width: 220,
                        height: 60
                    }
                ]
            };

    } catch (e) {
      console.log(e.message);
    }
  };

  PDF.prototype.setFooter = function(footer){
    try {
      if (!footer)
        throw new TypeError('PDF : setFooter - footer must be defined');
      this.footer = {
        //margin:  [20, 60],
        //height: 60,
        columns: [
          footer,
        ]
      };

    } catch (e) {
      console.log(e.message);
    }
  }

  PDF.prototype.setStyle = function(styles){
    try {
      if (!styles)
        throw new TypeError('PDF : setStyle - style must be defined');
      this.styles = styles;

    } catch (e) {
      console.log(e.message);
    }
  };

  PDF.prototype.setPageSize = function(pageSize){
    try {
      if (!pageSize)
        throw new TypeError('PDF : setPageSize - pageSize must be defined');
      this.pageSize = pageSize;

    } catch (e) {
      console.log(e.message);
    }
  };

  PDF.prototype.setPageMargins = function(pageMargins){
    try {
      if (!pageMargins)
        throw new TypeError('PDF : setPageMargins - pageMargins must be defined');
      this.pageMargins = pageMargins;

    } catch (e) {
      console.log(e.message);
    }
  };

  PDF.prototype.setInfo = function(info){
    try {
      if (!info)
        throw new TypeError('PDF : setInfo - info must be defined');
      this.info = info;

    } catch (e) {
      console.log(e.message);
    }
  }

  TSDOC_PDF.prototype.setDefaultUI = function(callback){
    var self = this;
    console.log(this);
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : setDefaultUI -This method needs a callback argument');
      this.setStyle();
      this.pdf.pageSize = 'A4';
      this.pdf.pageMargins = [40, 90, 40, 40];

      self.getFileTitle(function(fileTitle){
        try {
          if (fileTitle && fileTitle instanceof TypeError) {
            throw fileTitle;
          }else{
            self.pdf.info = {
                title: fileTitle,
                author: 'Milanostanze',
            };
          }

        } catch (e) {
          console.log(e.message);
        }
      });
      this.setBackground(function(){
        self.setHeader(function(){
          self.setFooter(function(){
            callback();
          });
        });
      });

    } catch (e) {
      console.log(e.message);
    }
  };

  TSDOC_PDF.prototype.setPDF = function(callback){
    var self = this;

    try {

      if(!this.intestatario_DocumentoFiscale)
        throw new TypeError('TSDOC_PDF - setPDF : this.intestatario_DocumentoFiscale undefined');

      if(!this.intestatario_DocumentoFiscale.DocumentoFiscale)
        throw new TypeError('TSDOC_PDF - setPDF : this.intestatario_DocumentoFiscale.DocumentoFiscale undefined');

      if(!this.intestatario_DocumentoFiscale.DocumentoFiscale.Documento)
        throw new TypeError('TSDOC_PDF - setPDF : this.intestatario_DocumentoFiscale.DocumentoFiscale.Documento undefined');

      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale.Documento;

      if(!doc.FatturaDettagli || !doc.Fattura_Componenti){
        this.intestatario_DocumentoFiscale.DocumentoFiscale.Documento.FatturaDettagli = null;
        this.intestatario_DocumentoFiscale.DocumentoFiscale.Documento.Fattura_Componenti = null;
        this.intestatario_DocumentoFiscale.DocumentoFiscale.Documento.LoadFD(function(){
          self.intestatario_DocumentoFiscale.DocumentoFiscale.Documento.LoadComponenti(function(){
            self.setDefaultUI(function(){
              self.setPDFContent(function(err){
                if(callback){
                  callback(err);
                }
              });
            });
          });
        });
      }else{
        this.setDefaultUI(function(){
          self.setPDFContent(function(err){
            if(callback){
              callback(err);
            }
          });
        });
      }

    } catch (e) {
      console.log(e.message);
      callback(e);
    }
  }

  TSDOC_PDF.prototype.setFtTopContent = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : setFtTopContent -This method needs a callback argument');
      var top = {
          table : null,
          layout: 'noBorders',
          style: 'contenuto',
      }

      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : setFtTopContent - intestatario_DocumentoFiscale undefined');

      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : setFtTopContent - doc error');
      var intestatario = this.intestatario_DocumentoFiscale.Intestatario;
      if (!intestatario || !intestatario.Intestatario)
        throw new TypeError('TSDOC_PDF : setFtTopContent - intestatario error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var customer = intestatario.Intestatario;

      switch (parseInt(customer.Type)) {
        case 1:
          try {
            var fd = invoice.FatturaDettagli.FatturaDettagli;
            if (!fd || !fd.length > 0)
              throw new TypeError('TSDOC_PDF : setFtTopContent - fd error');
            if(!invoice.getPDFMatrix() || invoice.getPDFMatrix() === "")
              throw new TypeError('TSDOC_PDF : setFtTopContent - Invoice Date error');
            var ins = new Inquilino_Stanza(fd[0].IdInquilinoStanze);
            ins.LoadRelationship(function(){
              var details = customer.Cliente;
              if(ins.Inquilino_Intestazione && ins.Inquilino_Intestazione.Id && parseInt(ins.Inquilino_Intestazione.Id) > 0){
                details = ins.Inquilino_Intestazione;
              }

              try {
                if (!details)
                  throw new TypeError('TSDOC_PDF : setFtTopContent - details error');
                if (!(details.CodiceFiscale || details.PartitaIva || details.ForeignIdentificationNumber))
                  throw new TypeError('TSDOC_PDF : setFtTopContent - fiscalCode error');

                details.getPDFMatrix(function(matrix){

                  try {
                    if (!matrix || !matrix.fullName)
                      throw new TypeError('TSDOC_PDF : setFtTopContent - Inquilino matrice error');
                    var apt = new Appartamento();
                    apt.LoadByIdStanza(ins.IdStanza, function(){
                      try {
                        if (!apt.Id || !(parseInt(apt.Id) > 0))
                          throw new TypeError('TSDOC_PDF : setFtTopContent - apt error');
                        apt.getPDFMatrix(function(m){
                          try {
                            if (!m || !m.top)
                              throw new TypeError('TSDOC_PDF : setFtTopContent - apt matrice error');
                            var table = {
                              widths: ['*', '*', 'auto'],
                              body: []
                            };

                            var dateRow = new leftRowBody();
                            dateRow.setDefaultStyle();
                            dateRow.text = invoice.getPDFMatrix();
                            table.body.push(dateRow.getRow());
                            table.body.push(emptyRow);

                            var gentile = new leftRowBody();
                            gentile.setDefaultStyle();
                            gentile.margin = [0, 10, 0, 0];
                            gentile.text = "Gent.le";
                            table.body.push(gentile.getRow());

                            var customerName = new leftRowBody();
                            customerName.setDefaultStyle();
                            customerName.margin = [0, 0];
                            customerName.text = matrix.fullName;
                            table.body.push(customerName.getRow());

                            var customerAddress = new leftRowBody();
                            customerAddress.setDefaultStyle();
                            customerAddress.margin = [0, -5];
                            customerAddress.text = matrix.street;
                            table.body.push(customerAddress.getRow());

                            var customerCity = new leftRowBody();
                            customerCity.setDefaultStyle();
                            customerCity.text = matrix.city;
                            table.body.push(customerCity.getRow());

                            table.body.push(emptyRow);

                            var aptStreet = new leftRowBody();
                            aptStreet.setDefaultStyle();
                            aptStreet.text = m.top;
                            table.body.push(aptStreet.getRow());

                            var aptCity = new leftRowBody();
                            aptCity.setDefaultStyle();
                            aptCity.margin = [0, -5];
                            aptCity.text = m.bottom;
                            table.body.push(aptCity.getRow());

                            table.body.push(emptyRow);

                            if(details.CodiceFiscale){
                              var fiscalCode = new leftRowBody();
                              fiscalCode.setDefaultStyle();
                              fiscalCode.bold = true;
                              fiscalCode.text = "CF: " + details.CodiceFiscale;
                              table.body.push(fiscalCode.getRow());
                            }else if(details.ForeignIdentificationNumber){
                              var fin = new leftRowBody();
                              fin.setDefaultStyle();
                              fin.bold = true;
                              fin.text = "Id Fiscale Iva : " + details.ForeignIdentificationNumber;
                              table.body.push(fin.getRow());
                            }

                            if(details.PartitaIva){
                              var pIva = new leftRowBody();
                              pIva.setDefaultStyle();
                              pIva.bold = true;
                              pIva.text = "P.IVA: " + details.PartitaIva;
                              table.body.push(pIva.getRow());
                            }

                            top.table = table;
                            self.pdf.content.push(top);
                            callback();

                          } catch (e) {
                            console.log(e.message);
                          }
                        });
                      } catch (e) {
                        console.log(e.message);
                      }
                    });
                  } catch (e) {
                    console.log(e.message);
                  }
                });
              } catch (e) {
                callback(e);
              }

            });
          } catch (e) {
            console.log(e.message);
          }
          break;
        case 2:
          var details = customer.Cliente;
          var table = {
            widths: ['*', '*', 'auto'],
            body: []
          };
          details.getPDFMatrix(function(matrix){
            var gentile = new leftRowBody();
            gentile.setDefaultStyle();
            gentile.margin = [0, 10, 0, 0];
            gentile.text = "Gent.le";
            table.body.push(gentile.getRow());

            var customerName = new leftRowBody();
            customerName.setDefaultStyle();
            customerName.margin = [0, 0];
            customerName.text = matrix.fullName;
            table.body.push(customerName.getRow());

            var customerAddress = new leftRowBody();
            customerAddress.setDefaultStyle();
            customerAddress.margin = [0, -5];
            customerAddress.text = matrix.street;
            table.body.push(customerAddress.getRow());

            var customerCity = new leftRowBody();
            customerCity.setDefaultStyle();
            customerCity.text = matrix.city;
            table.body.push(customerCity.getRow());

            table.body.push(emptyRow);

            if(details.PartitaIva){
              var pIva = new leftRowBody();
              pIva.setDefaultStyle();
              pIva.bold = true;
              pIva.text = "P.IVA: " + details.PartitaIva;
              table.body.push(pIva.getRow());
            }

            console.log(matrix);

            top.table = table;
            self.pdf.content.push(top);
            callback();
          });
          break;
      }

    } catch (e) {
      console.log(e.message);
    }
  }

  TSDOC_PDF.prototype.setFtMiddleContent = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : setFtMiddleContent -This method needs a callback argument');
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : setFtMiddleContent - intestatario_DocumentoFiscale undefined');

      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : setFtMiddleContent - doc error');
      var intestatario = this.intestatario_DocumentoFiscale.Intestatario;
      if (!intestatario || !intestatario.Intestatario)
        throw new TypeError('TSDOC_PDF : setFtMiddleContent - intestatario error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var customer = intestatario.Intestatario;

      switch (parseInt(invoice.Tipologia)) {
        case 4:
          var ftStorno = new Fattura_Storno(invoice.Id);
          ftStorno.Load(function(){
            if (ftStorno.IdFatturaStorno && (parseInt(ftStorno.IdFatturaStorno) > 0)){
              tipologia = "Nota di credito " + invoice.getNominativoPDFMatrix();
              var storno = new Fattura(ftStorno.IdFatturaStorno);
              storno.Load(function(){
                var tipologiaStorno = "Storno ";
                switch (parseInt(ftStorno.Tipologia)) {
                  case 1:
                    tipologiaStorno += " parziale " + storno.getNominativoPDFMatrix();
                    break;
                  case 2:
                    tipologiaStorno += " totale " + storno.getNominativoPDFMatrix();
                    break;
                }
                var middle = { stack: [
                                        { text: [
                                                 { text: tipologia, color: '#444444', bold: true, aligment: 'left', style: 'contenuto' }
                                              ],
                                          aligment : 'left'
                                        },
                                        { text: [
                                                 { text: tipologiaStorno, color: '#444444', aligment: 'left', style: 'contenuto' }
                                              ],
                                          aligment : 'left'
                                        }
                                    ],
                                margin: [0, 0, 0, 10]
                              };
                self.pdf.content.push(middle);
                callback();
              });
            }else{
              tipologia = "Nota di credito n. " + invoice.getNominativoPDFMatrix();
              var middle = { text: [
                                     { text: tipologia, color: '#444444', bold: true, aligment: 'left', style: 'contenuto'  }
                                  ],
                             aligment : 'left'
                           };

              self.pdf.content.push(middle);
              callback();
            }

          });
          break;
        default:
          tipologia = "Fattura n. " + invoice.getNominativoPDFMatrix();
          var middle = { text: [
                                 { text: tipologia, color: '#444444', bold: true, aligment: 'left', style: 'contenuto'  }
                              ],
                         aligment : 'left'
                       };

          self.pdf.content.push(middle);
          callback();
          break;
      }
    } catch (e) {
      console.log(e.message);
    }
  }

  TSDOC_PDF.prototype.getPaymentDetails = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : getPaymentDetails -This method needs a callback argument');
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : getPaymentDetails - intestatario_DocumentoFiscale undefined');

      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : getPaymentDetails - doc error');
      var intestatario = this.intestatario_DocumentoFiscale.Intestatario;
      if (!intestatario || !intestatario.Intestatario)
        throw new TypeError('TSDOC_PDF : getPaymentDetails - intestatario error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var customer = intestatario.Intestatario;

      switch (parseInt(invoice.Tipologia)) {
        case 4:
          callback();
          break;
        default:
          var pmtDetails = new Array();
          var r1 = {
            text: 'Modalità di pagamento:',
            color: '#5e5e5e',
            aligment: 'left',
            margin: [0, 0, 0, 10],
            style: 'contenuto'
          };
          pmtDetails.push(r1);
          var bankAccountDetails = new table();
          bankAccountDetails.widths = ['auto', '*', '*'];
          self.getBankAccountDetails(function(bad){
            try {
              if (!bad)
                throw new TypeError('TSDOC_PDF : getPaymentDetails - Bank Account Details error');
              pmtDetails.push(bad);
              self.getPurposeOfTransfer(function(pot){
                try {
                  if (!pot)
                    throw new TypeError('TSDOC_PDF : getPaymentDetails - Purpose of transfer error');
                  pmtDetails.push(pot);
                  self.pdf.content.push(pmtDetails);
                  callback();
                } catch (s) {
                  console.log(s.message);
                }
              });
            } catch (e) {
              console.log(e.message);
            }
          });
          break;
      }

    } catch (e) {
      console.log(e.message);
    }
  }

  TSDOC_PDF.prototype.getFtContent = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : getFtContent -This method needs a callback argument');
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : getFtContent - intestatario_DocumentoFiscale undefined');

      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : getFtContent - doc error');
      var intestatario = this.intestatario_DocumentoFiscale.Intestatario;
      if (!intestatario || !intestatario.Intestatario)
        throw new TypeError('TSDOC_PDF : getFtContent - intestatario error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var customer = intestatario.Intestatario;

      //BEGIN
      var fcs = invoice.Fattura_Componenti.Fattura_Componenti;
      var fcsMatrix = new Array();
      var fds = invoice.FatturaDettagli.FatturaDettagli;
      var hasDiscounts = invoice.FatturaDettagli.hasDiscounts();
      console.log(hasDiscounts);
      var fdsMatrix = new Array();

      var colTitle = {
          table : {
              widths: [ 250, '*', 100, '*', 100, '*', 100],
              body: [[{ text : "Descrizione" , alignment: 'left', bold : true }, '', { text: "Importo", alignment: 'left', bold : true }, '', { text: "Sconto", alignment: 'left', bold : true }, '', { text: "Totale", alignment: 'left', bold : true }]],
          },
          layout: 'noBorders',
          style: 'contenuto',
          color: '#5e5e5e',
      };

      var breakLine = {
          canvas: [
              {
                  type: 'line',
                  x1: 0,
                  y1: 5,
                  x2: 535,
                  y2: 5,
                  lineWidth: 0.5
              }
          ]
      };

      if(hasDiscounts){
        self.pdf.content.push(colTitle);
        self.pdf.content.push(breakLine);
      }

      for (var fc in fcs) {
        var matrix = fcs[fc].getPDFMatrix();
        var el = [{ text: matrix.Title , alignment: 'left' }, '', { text: matrix.Amount, alignment: 'left' }, '', '0,00€', '', { text: matrix.Amount, alignment: 'left' }];

        if(!hasDiscounts){
          el.splice(3,4);
        }

        fcsMatrix.push(el);
      }

      var size = 25;
      for (var i=0, x=0; i<fcsMatrix.length; i+=size, x++) {
          var smallarray = fcsMatrix.slice(i,i+size);
          var data_elems = {
              table : {
                  widths: [ 250, '*', 100, '*', 100, '*', 100],
                  body: [],

              },
              layout: 'noBorders',
              style: 'contenuto',
              color: '#5e5e5e',
          };

          if(x > 0){
              data_elems.pageBreak = 'before';
          }

          if(!hasDiscounts){
            data_elems.table.widths.splice(3,4);
          }

          data_elems.table.body = smallarray;
          self.pdf.content.push(data_elems);
      }
      //FINISH

      for (var fd in fds) {
        if (fds[fd].IdAttribuzione && parseInt(fds[fd].IdAttribuzione) !== 32) {
          var matrix = fds[fd].getPDFMatrix();
          var el = [{ text: matrix.Title , alignment: 'left' }, '', { text: matrix.Amount, alignment: 'left' }, '', { text: matrix.Discount, alignment: 'left' }, '', { text: matrix.Total, alignment: 'left' }];

          if(!hasDiscounts){
            el.splice(3,4);
          }

          fdsMatrix.push(el);
        }
      }

      var size = 25;
      for (var i=0, x=0; i<fdsMatrix.length; i+=size, x++) {
          var smallarray = fdsMatrix.slice(i,i+size);
          var data_elems = {
              table : {
                  widths: [ 250, '*', 100, '*', 100, '*', 100],
                  body: [],

              },
              layout: 'noBorders',
              style: 'contenuto',
              color: '#5e5e5e',
          };

          if(x > 0){
              data_elems.pageBreak = 'before';
          }

          if(!hasDiscounts){
            data_elems.table.widths.splice(3,4);
          }

          data_elems.table.body = smallarray;
          self.pdf.content.push(data_elems);
      }

      //IVA
      var iva = invoice.FatturaDettagli.getIva();
      if(iva){
        var ivaMatrix = iva.getPDFMatrix();
        var ivaRow = {
            table : {
                widths: [ 250, '*', 100, '*', 100, '*', 100],
                body: [],

            },
            layout: 'noBorders',
            style: 'contenuto',
            color: '#5e5e5e',
        };
        ivaRow.table.body = [[{ text: ivaMatrix.Title , alignment: 'left', margin: [0, 10, 0, 0], bold : true}, '', '', '', '', '', { text: ivaMatrix.Amount, alignment: 'left', margin: [0, 10, 0, 0], bold : true}]];

        if(!hasDiscounts){
          ivaRow.table.widths.splice(3,4);
          ivaRow.table.body[0].splice(3,4);
        }

        self.pdf.content.push(ivaRow);
      }

      var totaleText = "Totale importo dovuto";
      var tot = {
          table : {
              widths: [ 250, '*', 100, '*', 100, '*', 100],
              body: [],

          },
          layout: 'noBorders',
          style: 'contenuto',
          color: '#5e5e5e',
      };
      tot.table.body = [[{ text: totaleText , alignment: 'left', margin: [0, 10, 0, 0], bold : true}, '', '', '', '', '', { text: invoice.getTotaleMatrix(), alignment: 'left', margin: [0, 10, 0, 0], bold : true}]];

      if(!hasDiscounts){
        tot.table.widths.splice(2,4);
        tot.table.body[0].splice(2,4);
      }

      self.pdf.content.push(tot);

      if(invoice.ivaExists()){
        var soggettoIvaText = "* Soggetto ad IVA";
        var sIva = {
            table : {
                widths: [ 'auto', '*', 100],
                body: [],

            },
            layout: 'noBorders',
            style: 'contenuto',
            color: '#5e5e5e',
        };
        sIva.table.body = [[{ text: soggettoIvaText , alignment: 'left', margin: [0, 10, 0, 0], bold : true}, '', '']];
        self.pdf.content.push(sIva);
      }

      callback();

    } catch (e) {
      console.log(e.message);
    }
  }

  TSDOC_PDF.prototype.getFileTitle = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : getFileTitle -This method needs a callback argument');
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : getFileTitle - intestatario_DocumentoFiscale undefined');
      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : getFileTitle - doc error');
      var intestatario = this.intestatario_DocumentoFiscale.Intestatario;
      if (!intestatario || !intestatario.Intestatario)
        throw new TypeError('TSDOC_PDF : getFileTitle - intestatario error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var customer = intestatario.Intestatario;

      var title = "";
      var date = new Date(invoice.Data);

      title += invoice.Numero + "-";
      title += date.getFullYear().toString().substr(2,2);
      title += " " + intestatario.getNominativo();

      callback(title);

    } catch (e) {
      console.log(e.message);
      callback(e);
    }
  }

  TSDOC_PDF.prototype.setFtBottomContent = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : setFtBottomContent -This method needs a callback argument');
      this.getPaymentDetails(function(){
        self.getFtContent(function(){
          callback();
        });
      });
    } catch (e) {
      console.log(e.message);
    }
  }

  TSDOC_PDF.prototype.setPDFContent = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : setPDFContent -This method needs a callback argument');
      this.pdf.content = new Array();

      this.setFtTopContent(function(err){
        try {
          if (err && err instanceof TypeError)
            throw err;
          self.setFtMiddleContent(function(){
            self.setFtBottomContent(function(){
              callback();
            });
          });
        } catch (e) {
          callback(e);
        }
      });
    } catch (e) {
      callback(e);
    }
  };

  TSDOC_PDF.prototype.setBackground = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : setBackground -This method needs a callback argument');
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : setBackground - intestatario_DocumentoFiscale undefined');

      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : setBackground - doc error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var img = Backgrounds[invoice.Societa].content;
      if (!img)
        throw new TypeError('TSDOC_PDF : setBackground - img error');
      this.pdf.setBackground(img);
      callback();
    } catch (e) {
      console.log(e.message);
    }
  };

  TSDOC_PDF.prototype.setHeader = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : setHeader -This method needs a callback argument');
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : setHeader - intestatario_DocumentoFiscale undefined');

      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : setHeader - doc error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var img = Logos[invoice.Societa].content;
      if (!img)
        throw new TypeError('TSDOC_PDF : setHeader - img error');
      this.pdf.setHeader(img);
      callback();
    } catch (e) {
      console.log(e.message);
    }
  };

  TSDOC_PDF.prototype.setFooter = function(callback){
    var self = this;
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : setFooter -This method needs a callback argument');
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : setFooter - intestatario_DocumentoFiscale undefined');

      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : setFooter - doc error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var obj = Footer[invoice.Societa].content;
      if (!obj)
        throw new TypeError('TSDOC_PDF : setFooter - obj error');
      this.pdf.setFooter(obj);
      callback();
    } catch (e) {
      console.log(e.message);
    }
  };

  TSDOC_PDF.prototype.getPurposeOfTransfer = function(callback){
    var self = this;
    var pot = new table();
    pot.widths = ['auto'];
    pot.layout = 'noBorders';
    pot.style = 'contenuto';
    pot.lineHeight = 1;
    pot.body = new Array();
    var row = [{  text: 'Causale da indicare nella disposizione di bonifico: ' , color: '#5e5e5e', alignment: 'left',  margin: [0, 10, 0, 0] }];
    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : getPurposeOfTransfer -This method needs a callback argument');
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : getPurposeOfTransfer - intestatario_DocumentoFiscale undefined');
      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : getPurposeOfTransfer - doc error');
      var intestatario = this.intestatario_DocumentoFiscale.Intestatario;
      if (!intestatario || !intestatario.Intestatario)
        throw new TypeError('TSDOC_PDF : getPurposeOfTransfer - intestatario error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var customer = intestatario.Intestatario;
      var causale = "";
      if (invoice.Causale && invoice.Causale !== "") {
        causale = invoice.Causale;
      }else{
        var nominativo = customer.getNominativo();
        var ftNominativo = invoice.getNominativoPDFMatrix();
        try {
          if (!nominativo)
            throw new TypeError('TSDOC_PDF : getPurposeOfTransfer - Customer Nominativo Text error');
          if (!ftNominativo)
            throw new TypeError('TSDOC_PDF : getPurposeOfTransfer - Doc Nominativo Text error');
          causale = "Ft. " + ftNominativo + " RIF. " + nominativo;
        } catch (e) {
          console.log(e.message);
        }
      }

      if(!causale || causale === "")
        throw new TypeError('TSDOC_PDF : getPurposeOfTransfer - Causale Text error');
      /*var row = [
              [ '', '', '' ],
              [ '', '', '' ],
              [ { text: 'Causale da indicare nella disposizione di bonifico: ', color: '#5e5e5e' }, '', ''],
              [ { text: causale, color: '#5e5e5e', bold : true }, '', '' ]
      ];*/
      row.push({  text: causale , color: '#5e5e5e', alignment: 'left',  margin: [0, 0, 0, 10], bold : true });

      pot.body.push(row);
      callback(pot.getTable());
    } catch (e) {
      console.log(e.message);
    }
  }

  TSDOC_PDF.prototype.getBankAccountDetails = function(callback){
    var self = this;
    var bad = new table();
    bad.widths = ['auto', '*', '*'];
    bad.layout = 'noBorders';
    bad.lineHeight = 1;
    bad.style = 'contenuto';

    try {
      if (!callback)
        throw new TypeError('TSDOC_PDF : getBankAccountDetails -This method needs a callback argument');
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : getBankAccountDetails - intestatario_DocumentoFiscale undefined');
      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : getBankAccountDetails - doc error');
      var intestatario = this.intestatario_DocumentoFiscale.Intestatario;
      if (!intestatario || !intestatario.Intestatario)
        throw new TypeError('TSDOC_PDF : getBankAccountDetails - intestatario error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var customer = intestatario.Intestatario;
      var scadenza = invoice.getScadenza();

      switch (parseInt(invoice.Societa)) {
        case 0:
          var b = [
                  [ { text: 'Bonifico Bancario intestato a Finlibera s.p.a.', color: '#5e5e5e' }, '', ''],
                  [ { text: 'IBAN: IT80S0100501606000000000812', color: '#5e5e5e' }, '', '' ],
                  [ { text: 'Bic/Swift: BNLIITRR', color: '#5e5e5e' }, '', '' ],
                  [ { text: 'Nome Banca: Banca Nazionale Del Lavoro', color: '#5e5e5e' }, '', '' ]
          ];
          if (parseInt(invoice.Tipologia) === 2 && scadenza) {
            b.push([ { text: 'Scadenza di pagamento il ' + scadenza, color: '#5e5e5e', bold : true }, '', '' ]);
          }
          bad.body = new Array();
          bad.body.push(b);
          callback(bad.getTable());
          break;
        case 1:
          var b = [
                  [ { text: 'Bonifico Bancario intestato a Ecolibera Srl', color: '#5e5e5e' }, '', ''],
                  [ { text: 'IBAN: IT88T0100501606000000001225', color: '#5e5e5e' }, '', '' ],
                  [ { text: 'Bic/Swift: BNLIITRR', color: '#5e5e5e' }, '', '' ],
                  [ { text: 'Nome Banca: Banca Nazionale Del Lavoro', color: '#5e5e5e' }, '', '' ]
          ];
          if (parseInt(invoice.Tipologia) === 2 && scadenza) {
            b.push([ { text: 'Scadenza di pagamento il ' + scadenza, color: '#5e5e5e' }, '', '' ]);
          }
          bad.body = new Array();
          bad.body.push(b);
          callback(bad.getTable());
          break;
      }

    } catch (e) {
      console.log(e.message);
    }
  }

  TSDOC_PDF.prototype.setStyle = function(){
    var self = this;
    try {
      if (!this.intestatario_DocumentoFiscale || !(this.intestatario_DocumentoFiscale instanceof Intestatario_DocumentoFiscale) || !this.intestatario_DocumentoFiscale.DocumentoFiscale || !this.intestatario_DocumentoFiscale.Intestatario)
        throw new TypeError('TSDOC_PDF : setStyle - intestatario_DocumentoFiscale undefined');

      var doc = this.intestatario_DocumentoFiscale.DocumentoFiscale;
      if (!doc || !doc.Documento || !(doc.Documento instanceof Fattura))
        throw new TypeError('TSDOC_PDF : setStyle - doc error');

      /**
       * [invoice description]
       * @type {[Fattura]}
       */
      var invoice = doc.Documento;
      var obj = Style[invoice.Societa].content;
      if (!obj)
        throw new TypeError('TSDOC_PDF : setStyle - obj error');
      this.pdf.setStyle(obj);
    } catch (e) {
      console.log(e.message);
    }
  };

})();
